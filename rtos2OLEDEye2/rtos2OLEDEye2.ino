...
void drawNeutralEyes() {
  ...
  int leftEyeX = 32, rightEyeX = 96, eyeY = 30;
  display.fillRoundRect(leftEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillRoundRect(rightEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillCircle(leftEyeX, eyeY, 12, SSD1306_BLACK);
  display.fillCircle(rightEyeX, eyeY, 12, SSD1306_BLACK);
  display.display();
}
...
void drawLeftEyes() {
  display.clearDisplay();
  int eyeWidth = 40, eyeHeight = 25;
  int leftEyeX = 27, rightEyeX = 91, eyeY = 30;
  display.fillRoundRect(leftEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillRoundRect(rightEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillCircle(leftEyeX - 6, eyeY, 12, SSD1306_BLACK);
  display.fillCircle(rightEyeX - 6, eyeY, 12, SSD1306_BLACK);
  display.display();
}
...
void drawRightEyes() {
  display.clearDisplay();
  int eyeWidth = 40, eyeHeight = 25;
  int leftEyeX = 37, rightEyeX = 101, eyeY = 30;
  display.fillRoundRect(leftEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillRoundRect(rightEyeX - 20, eyeY - 12, eyeWidth, eyeHeight, 10, SSD1306_WHITE);
  display.fillCircle(leftEyeX + 6, eyeY, 12, SSD1306_BLACK);
  display.fillCircle(rightEyeX + 6, eyeY, 12, SSD1306_BLACK);
  ...
  for (;;) {
    ...
    if (xQueueReceive(qFrames, &frame, portMAX_DELAY) == pdTRUE && frame != nullptr) {
      ...
      if (run_classifier(&signal, &result) == EI_IMPULSE_OK) {
        ...
        if (bestp > CONF) {
          if (is_right) {
            // Right turn: right motor forward, left reverse
            digitalWrite(PIN_RIGHT, HIGH);
            digitalWrite(PIN_LEFT, LOW);
            showEyeMovement("right");
          } else if (is_left) {
            // Left turn: left motor forward, right reverse
            digitalWrite(PIN_LEFT, HIGH);
            digitalWrite(PIN_RIGHT, LOW);
            showEyeMovement("left");
          }
        } else {
          // Stop motors + neutral eyes
          digitalWrite(PIN_RIGHT, LOW);
          digitalWrite(PIN_LEFT, LOW);
          drawNeutralEyes();
        }
