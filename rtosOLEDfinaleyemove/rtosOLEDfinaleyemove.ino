...
static EyeState currentEyeState = NEUTRAL;
static unsigned long lastEyeUpdate = 0;
static unsigned long lastCommandTime = 0;
static const unsigned long COMMAND_DISPLAY_TIME = 2000; // Show command for 2 seconds
static const unsigned long EYE_UPDATE_INTERVAL = 100;   // Update eyes every 100ms for smooth animation
...
void drawEyes(int pupilOffset, bool blink) {
  ...
  int eyeWidth = 40, eyeHeight = 25, eyeY = 30;
  ...
  if (!blink) {
    // Draw pupils with natural offset
    int pupilSize = 12;
    int leftPupilX = leftEyeX + pupilOffset;
    int rightPupilX = rightEyeX + pupilOffset;
    
    // Constrain pupils within eyes
    leftPupilX = constrain(leftPupilX, leftEyeX - 8, leftEyeX + 8);
    rightPupilX = constrain(rightPupilX, rightEyeX - 8, rightEyeX + 8);
    
    display.fillCircle(leftPupilX, eyeY, pupilSize, SSD1306_BLACK);
    display.fillCircle(rightPupilX, eyeY, pupilSize, SSD1306_BLACK);
    
    // Draw eye highlights (small white circles)
...
void updateEyes() {
  ...
  // Handle blinking
  if (!blinking && random(1000) < 5) { // Random blink ~0.5% chance per update
    blinking = true;
  ...
  // Handle command display timeout
  if (currentEyeState != NEUTRAL && currentTime - lastCommandTime > COMMAND_DISPLAY_TIME) {
  ...
  if (currentPupilOffset != targetPupilOffset) {
    if (currentPupilOffset < targetPupilOffset) {
      currentPupilOffset += 1;
    } else {
      currentPupilOffset -= 1;
    }
  } else if (currentEyeState == NEUTRAL) {
    // Subtle natural eye movements when neutral
    if (random(100) < 3) { // 3% chance to make small random movement
      targetPupilOffset = random(-3, 4);
    }
  }
...
void setEyeCommand(const char* command) {
  lastCommandTime = millis();
  
  if (strcmp(command, "left") == 0) {
    currentEyeState = LOOKING_LEFT;
    targetPupilOffset = -8; // Look left
  } else if (strcmp(command, "right") == 0) {
    currentEyeState = LOOKING_RIGHT;
    targetPupilOffset = 8; // Look right
  } else if (strcmp(command, "forward") == 0 || strcmp(command, "backward") == 0) {
    currentEyeState = NEUTRAL;
    targetPupilOffset = 0; // Look forward for forward/backward commands
    // Quick blink acknowledgement
    blinking = true;
  ...
  for (;;) {
    ...
    if (xQueueReceive(qFrames, &frame, portMAX_DELAY) == pdTRUE && frame != nullptr) {
      ...
      if (run_classifier(&signal, &result) == EI_IMPULSE_OK) {
        ...
        // Update eye animation based on command
        if (bestp > CONF) {
          setEyeCommand(best);
        }
    ...
    // Update eye animation periodically
    unsigned long currentTime = millis();
    if (currentTime - lastEyeUpdate >= EYE_UPDATE_INTERVAL) {
      updateEyes();
...
void loop() {
  // Update eye animation in main loop too for smoothness
  unsigned long currentTime = millis();
  if (currentTime - lastEyeUpdate >= EYE_UPDATE_INTERVAL) {
    updateEyes();
    lastEyeUpdate = currentTime;
