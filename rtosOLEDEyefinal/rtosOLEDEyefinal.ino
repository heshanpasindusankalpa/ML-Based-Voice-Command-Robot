  ...
  for (;;) {
    ...
    if (xQueueReceive(qFrames, &frame, portMAX_DELAY) == pdTRUE && frame != nullptr) {
      ...
      if (run_classifier(&signal, &result) == EI_IMPULSE_OK) {
        ...
        // OLED eyes instead of text
        if (bestp > CONF) {
          if (is_left) drawLeftEyes();
          else if (is_right) drawRightEyes();
          else drawNeutralEyes();
        } else {
          drawNeutralEyes();
        }
